...
(function mountPerfWidget() {
  ...
  if (!mgr || !React || !ReactDOM) return; // perf.js / CDN scripts missing — skip silently
  ...
  function PerfHUD() {
    ...
    useEffect(() => {
      const onKey = (e: KeyboardEvent) => {
        if (e.shiftKey && (e.key === 'P' || e.key === 'p')) setVisible((v: boolean) => !v);
      };
    ...
    useEffect(() => {
      ...
      // toggling the HUD back on is current, not whatever it was the
      // moment it got hidden — the listener itself is essentially free.
    ...
    return h(
      ...
      h('span', null, 'PERF'),
      h('span', { style: { opacity: 0.5, margin: '0 6px' } }, '\u00b7'),
      h('span', null, String(quality).toUpperCase()),
      h('span', { style: { opacity: 0.35, marginLeft: '8px' } }, 'shift+p to hide')
    );
  }
